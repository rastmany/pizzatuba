...
const StickyNav: React.FC<StickyNavProps> = ({ activeCategory, onCategoryClick }) => {
  ...
  return (
    <motion.nav
      ...
      className={`sticky top-16 md:top-20 z-40 w-full transition-all duration-300 ${
        isSticky ? 'backdrop-blur-xl py-2 border-b border-gray-100' : 'py-4'
      }`}
    >
        ...
        <div className="flex items-center gap-3">
          {categories.map((cat) => (
            <button
              ...
              className={`whitespace-nowrap px-6 py-2.5 rounded-full text-sm font-black transition-all duration-300 transform active:scale-95 ${
                activeCategory === cat 
